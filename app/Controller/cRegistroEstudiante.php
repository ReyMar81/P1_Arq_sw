...
class cRegistroEstudiante
{
    ...
    public function cargar_csv(): void
    {
        ...
        $resultado = $this->modelo_re->insertar_registro([
            'op'       => 'csv',
            'contenido' => $contenido,
        ...
        if ($resultado['error'] !== null) {
            $this->redirect($resultado['error'], 'error');
        }

        $_SESSION['reg_staging'] = $resultado['staging'];
    ...
    public function guardar(): void
    {
        ...
        $estudiantes_id  = $_SESSION['reg_staging']  ?? [];
        ...
        // Llamar al modelo para manejar la lógica transaccional
        $resultado = $this->modelo_re->insertar_registro($estudiantes_id, $grupo_id);

        // Validar resultado
        if ($resultado['error'] !== null) {
            $this->redirect('Error en la transacción. Ningún dato fue guardado. Detalle: ' . $resultado['error'], 'error');
        }
        ...
        $msg = $resultado['guardados'] . " estudiante(s) registrado(s) exitosamente en el grupo.";
        if ($resultado['omitidos'] > 0) {
            $msg .= " (" . $resultado['omitidos'] . " omitido(s) porque ya estaban registrados).";
        }
        $this->redirect($msg);
    }
